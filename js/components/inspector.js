(function () {
  ...
  function renderFileTree(items, level, parentPath) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      if (item.type === 'dir') {
        ...
        html += '<i class="fas ' + icon + '" style="width:16px;text-align:center;color:var(--warning);flex-shrink:0"></i>';
        ...
        html += '<div class="tree-item tree-file" data-path="' + fullPath + '" data-file="' + item.name + '" style="display:flex;align-items:center;gap:6px;padding:4px ' + padding + 'px 4px 8px;cursor:pointer;border-radius:var(--radius-sm);transition:var(--transition);font-size:0.85rem">';
  ...
  function renderFileContent(project, filePath, fileName) {
    var files = project.generatedFiles || {};
    var content = files[fileName] || null;
    var html = '';
    if (!content) {
      for (var key in files) {
        if (files.hasOwnProperty(key) && filePath && filePath.indexOf(key) !== -1) {
          content = files[key];
          break;
        }
      }
    ...
    html += '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;padding-bottom:8px;border-bottom:1px solid var(--border)">';
    ...
    if (content === null) {
      ...
      html += '<div class="code-header"><span class="code-lang"><i class="fas fa-code"></i> ' + escHtml(fileName || '') + '</span><div class="code-actions"><button class="btn btn-sm btn-ghost copy-file-btn" style="font-size:0.7rem"><i class="fas fa-copy"></i> Копировать</button></div></div>';
  ...
  App.registerComponent({
    ...
    render: function (container) {
      ...
      var structure = App.projectManager ? App.projectManager.getProjectStructure(project) : [];
      ...
      html += '</div></div>';
      html += '<div class="split-view" style="flex:1;border-top:1px solid var(--border)">';
      html += '<div class="split-left" style="flex:0 0 40%;max-width:50%;display:flex;flex-direction:column;padding:0;border-right:1px solid var(--border);overflow:hidden">';
